...
Prompt: Build a product feedback form. Users should provide their name, email, and feedback text. Add a rating dropdown with options 1 to 5 stars. Include a submit button that displays all submitted feedback entries below the form. After a successful submission, the form fields should be cleared and remain empty until the user starts a new entry. Each entry should show name, rating, and feedback text.
...
| 6   | state       | Clear form fields after successful submission and keep them empty      | minor  | Clearing and maintaining empty form fields after submission resets the application state, preparing the form for the next feedback entry without requiring manual user clearing. | C5           |
